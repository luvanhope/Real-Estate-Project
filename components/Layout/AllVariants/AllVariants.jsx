              ...
              if (isFavorite) {
                ...
                dispatch(
                  addToFavorites({
                    ...
                    img: item.images?.[0]?.srcUrl || "/no-image.png",
                    branchName: item.customer?.branchName,
                    Size: item.displaySize,
                    adress: item.displayAddress,
                  }),
                );
              }
            };
            ...
            return (
              <div key={item.id} className="relative">
                ...
                <button
                  ...
                  className="absolute top-6 right-6 p-2 bg-white rounded-full shadow-md hover:scale-110 transition-transform"
                >
                  {isFavorite ? (
                    <FaHeart className="text-red-500 text-xl" /> // Активное состояние
                  ) : (
                    <FaRegHeart className="text-gray-400 text-xl" /> // Пустое состояние
                  )}
                </button>
              </div>
            );
